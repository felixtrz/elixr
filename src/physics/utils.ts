...
export const createTrimesh = (geometry: THREE.BufferGeometry) => {
	const vertices = geometry.clone().toNonIndexed().attributes.position
		.array as number[];
	const indices = Object.keys(vertices).map(Number);
	return new Physics.Trimesh(vertices, indices);
};

export const createConvexPolyhedron = (geometry: THREE.BufferGeometry) => {
	...
	for (let i = 0, il = vertices.length; i < il; i++) {
		...
		if (verticesMap[key] === undefined) {
			verticesMap[key] = i;
			points.push(
				new Physics.Vec3(vertices[i].x, vertices[i].y, vertices[i].z),
			);
	...
	return new Physics.ConvexPolyhedron({
		vertices: points,
		faces: cannonFaces,
	});
};

export const offsetCenterOfMass = (
	body: Physics.Body,
	centreOfMass: Physics.Vec3,
) => {
	...
	const worldCenterOfMass = new Physics.Vec3();
